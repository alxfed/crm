...
companies_created_rerun_path = '/media/alxfed/toca/aa-crm/uploads/companies_created_rerun.csv'

headers = {"Content-Type": "application/json"}
querystring = {"hapikey": API_KEY}

already_created = pd.read_csv(companies_created_path)['Name']
...
with open(companies_to_create_path) as f:
    ...
    for row in f_csv:
        list_of_properties = []
        if row['Name'] not in already_created.values:
            for key in row:
            ...
            print(row['Name'], ' - already created')

with open(companies_created_rerun_path,'w') as f:
